# 11.嵌入式计算机视觉:在 Raspberry Pi 上运行 OpenCV 程序
...
树莓 Pi 的制造商最近还发布了一个定制的相机板，以鼓励 Pi 的图像处理和计算机视觉应用。该板很小，重量只有 3 克，但它拥有一个 500 万像素的 CMOS 传感器。它使用带状电缆连接到 Pi，看起来如图 [11-6](#Fig6) 所示。
...
还记得我们在[第 5 章](05.html)中完善的基于颜色的物体检测器吗(清单 5-7)？让我们看看它是如何运行的 USB 摄像头与彩色图像抓取使用包装代码。图 [11-8](#Fig8) 显示，对于尺寸为 320 x 240 的帧，如果我们使用包装代码抓取帧，检测器的运行速度会快 50 %!
