# 5.基于 ASP.NET MVC 核心的 Web 应用中的计算机视觉
...
### 用 Visual Studio for Mac 创建 Web 应用程序
...
注意`asp-controller`是如何指定关联的控制器类的(省略了`Controller`的字面意思)以及`asp-action`是如何允许你在控制器中指定与页面关联的动作的。现在页面已经准备好了，您可以测试应用程序了。
