...
为了保持所有工人之间的变量同步，它使用`CollectiveOps`作为 all-reduce 通信方法。集合运算是TensorFlow图中的单个运算。它可以根据硬件、网络拓扑和张量大小在 TensorFlow 运行时自动选择 all-reduce 算法。
