...
第 6 到 10 行通过调用位于目录`models/research/object_detection`中的脚本`export_inference_graph.py`来导出TensorFlow图。该脚本采用以下参数:
...
2.  编写 Python 代码，该代码将利用导出的TensorFlow图(导出的模型)来预测新图像中未包含在训练集或测试集中的对象。
