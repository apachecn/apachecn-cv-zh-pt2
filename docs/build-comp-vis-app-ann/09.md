...
在本节中，我们将首先检查用于训练和测试表面缺陷检测模型的数据集。我们将把图像和注释转换成 TFRecord 文件，并在 Google Colab 上训练一个 SSD 模型。我们将应用第 [6](06.html) 章中介绍的目标检测概念。
...
我们现在将编辑`pipeline.config`文件，如第 [6](06.html) 章“配置对象检测流水线”一节所述。清单 [9-5](#PC6) 显示了根据当前配置编辑的`pipeline.config`文件的各个部分。
...
为了利用该模型进行实时检测，我们需要导出 TensorFlow 图。查看第 [6](06.html) 章的“导出 TensorFlow 图”部分，了解详细信息。
